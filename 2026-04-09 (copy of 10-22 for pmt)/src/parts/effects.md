...
<!--Итак самая основная деталь реактивной системы Vue это ReactiveEffect (пример кода)
так вот сам ReactiveEffect занимается сбром и сохранением списка отслеживаемых зависимостей, допускает принудительный запуск с помощью определенной функции (это уже относится к его внутреннему api effect), сигнализирует о потребности в перерасчете и перед этим помечается как "грязный" (как например в случае вычисляемых свойств) 
то есть когда внутренняя зависимость изменилась, то сам эффект не пересчитывается мгновенно а лишь помечается как грязный-->
...
<!--Далее effectScope - нередко из-за непонимая разработчики задумываются а почему все вотчеры в скоупе например конкретного компонента больше не исполняются, хотя явной очистки при размонтировании как в случае каких нибудь ивентов мы не делали
этому способствует такой механизм как effectScope -->
...
<!--у effectScope так же есть вспомогательныя утилита для получения текущего скоупа исполнения и хук onScopeDispose - который довольно часто используется для явного вызова эффектов в композаблах или утилитах при очистке активного скоупа-->
