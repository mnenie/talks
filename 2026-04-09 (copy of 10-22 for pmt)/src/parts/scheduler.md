...
<!--Итак у нас есть шедулер - он же планировщик (механизм который синхронизирует обновления, и изменения планировщик производит в конце очереди микротасков (выполняются отдельные батчи)), у него есть 3 режима работы - pre, sync, post. Ну и на самом деле не совсем так, sync - мы можем вынести за работу планировщика потому что это синхронные операции, далее происходят операции в pre режиме - то есть до обновления DOM (режим по умолчанию), далее обновляется DOM, и далее срабатывают post операции (после обновления DOM) и так циклично -->
